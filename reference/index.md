# Package index

## Validate, read & write OME-Zarr data in R

- [`ome_read()`](https://huber-group-embl.github.io/romeo/reference/ome_read.md)
  : Read a multiscale OME-Zarr file
- [`ome_validate()`](https://huber-group-embl.github.io/romeo/reference/ome_validate.md)
  : Validate a multiscale OME-Zarr file
- [`ome_write()`](https://huber-group-embl.github.io/romeo/reference/ome_write.md)
  : ome_write

## Manipulating `ome_zarr` objects

- [`plot(`*`<ome_zarr>`*`)`](https://huber-group-embl.github.io/romeo/reference/plot.ome_zarr.md)
  :

  Plot an `ome_zarr` object.

- [`` `[`( ``*`<ome_zarr>`*`)`](https://huber-group-embl.github.io/romeo/reference/sub-.ome_zarr.md)
  :

  Subset an `ome-zarr` object

- [`extract_levels()`](https://huber-group-embl.github.io/romeo/reference/extract_levels.md)
  :

  Extract specific levels from a multiscale `ome-zarr` object
